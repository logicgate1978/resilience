    ...
    for svc in services:
        ...
        if service_name == "ec2" and action in ("stop", "reboot", "terminate"):
            ...
            if use_fis:
                ...
                raise ValueError(
                    f"ec2:{action} with use_fis=false requires a custom EC2 handler, but none is registered."
                )
            else:
                custom_services.append(svc)
            continue
        if service_name == "rds" and action in ("reboot", "failover"):
            ...
            if use_fis:
                ...
                raise ValueError(
                    f"rds:{action} with use_fis=false requires a custom RDS handler, but none is registered."
                )
            else:
                custom_services.append(svc)
            continue
