...
usage() {
  cat <<'EOF'
Usage:
  ./commands/s3/destroy_s3_replication_stack.sh [--name <base-name>] [--primary-region <aws-region>] [--secondary-region <aws-region>]

Notes:
  - If commands/s3/.state/current_s3_replication_stack.txt exists, the script reads names from it.
  - CLI arguments override the values from the state file when supplied.
  - The script removes bucket replication, empties both buckets, deletes them, removes the IAM role policy and role, and then deletes the local state file.
EOF
}
...
echo
echo "Destroying S3 replication resources..."
echo
...
echo
echo "S3 replication stack has been removed."
